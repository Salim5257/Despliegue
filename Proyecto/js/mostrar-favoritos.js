$(document).ready(function () {
                    ...
                    template += `
                        <tr task-prodId="${task.id}" style="border-bottom: white solid 10px;">
                            ...
                            <td style="justify-content: center; text-align: center; vertical-align: middle; width: 5%;">
                                <button type="submit" class="taskprod-addcart btn btn-success" style="font-size: 2rem; margin: 1rem;"><i class="fas fa-shopping-cart"></i></button>
    ...
    //Funcion que me permite añadir un producto al carrito por su id sin necesidad de recargar la pagina.
    $(document).on('click', '.taskprod-addcart', function () {
        //Muestro un alert para confirmar añadir el producto.
        if (confirm('¿Quieres añadir este producto al carrito?')) {
            let element = $(this)[0].parentElement.parentElement;
            let id = $(element).attr('task-prodId');
            $.post('../carrito/cart-add.php', {id}, function (response) {
                verFavoritos(response);
            });
        }
    });
