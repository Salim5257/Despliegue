$(document).ready(function () {
    ...
    function verCarrito() {
        ...
        $.ajax({
            url: '../carrito/cart-list.php', //le pasamos la url del archivo php que extrae los datos de la base de datos por medio de consultas
            ...
            success: function (response) {
                        ...
                        <tr task-prodId="${task.id}" style="border-bottom: white solid 10px;">
                            ...
                            <td style="justify-content: center; text-align: center; vertical-align: middle; width: 5%;">
                                <form action="../favoritos/favs-add.php" method="post">
                                    ...
                                    <button type="submit" class="btn btn-success" style="font-size: 2rem; margin: 1rem; width: 4rem;"><i class="fas fa-heart"></i></button>
                                </form>
                ...
                $('#mostrarCart').html(template); //Los datos que se muestren apareceran en la parte que contenga la id #mostrar
            }
        });
    }
    ...
    $(document).on('click', '.taskprod-delete', function () {
        ...
        if (confirm('¿Quieres eliminar este producto?')) {
            ...
            $.post('cart-delete.php', {id}, function (response) {
                location.reload();
                verCarrito(response);
            });
        }
    ...
    function searchCart() {
        ...
        $('#searchCart').keyup(function() { 
            if($('#searchCart').val()) {
            let search = $('#searchCart').val();
            $.ajax({
                url: '../app/search-php/search-cart.php', 
                data: {search}, 
                type: 'POST', 
                success: function (response) {
                if(!response.error) { 
